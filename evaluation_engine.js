...
function buildPtStructure(item) {
  ...
  const spec = String((item && (item.SPECIFICATION || item.MODEL_TYPE)) || (trInfoItem && (trInfoItem.SPECIFICATION || trInfoItem.MODEL_TYPE)) || '').toUpperCase();
  const is3Winding = noWindingStr === '3' || (lv2Rated !== '' && lv2Rated !== '-' && lv2Rated !== '0') || spec.includes('3W');
  ...
  if (is3Winding) {
    activePartSubs.push({
      sub: 'TV Winding', full: 100, subWeight: 30,
      methods: [
