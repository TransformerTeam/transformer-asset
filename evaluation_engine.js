...
function buildPtStructure(item) {
  ...
  const hvWindingSub = {
    sub: 'HV Winding', full: 100, subWeight: (is3Winding ? 30 : 45),
    methods: [
  ...
  const lvWindingSub = {
    sub: 'LV Winding', full: 100, subWeight: (is3Winding ? 30 : 45),
    methods: [
  ...
  const tvWindingSub = {
    sub: 'TV Winding', full: 100, subWeight: (isAuto ? 45 : 30),
    methods: [
  ...
  if (isAuto) {
    ...
    activePartSubs.push(lvWindingSub);
    activePartSubs.push(tvWindingSub);
  } else {
    activePartSubs.push(lvWindingSub);
  }
    ...
    if (latestWindingPf) {
      ...
      if (isAuto) {
        ...
        if (isLv) {
          ...
          if (ct !== null || cth !== null || (maxTvChange !== null && maxTvChange > 0)) {
            hasValidData = true;
        ...
        if (isTv) {
          ...
          const maxHvChange = isValidNum(latestWindingPf.maxchv_change) ? parseFloat(latestWindingPf.maxchv_change) : null;
          if (ch !== null || chl !== null || (maxHvChange !== null && maxHvChange > 0)) {
            hasValidData = true;
            errFat = maxHvChange !== null ? Math.abs(maxHvChange) : 0;
            const chStr = ch !== null ? `${ch.toFixed(1)} pF` : '-';
        ...
        if (isTv) {
          ...
          const maxLvChange = isValidNum(latestWindingPf.maxclv_change) ? parseFloat(latestWindingPf.maxclv_change) : null;
          if (cl !== null || clh !== null || (maxLvChange !== null && maxLvChange > 0)) {
            hasValidData = true;
            errFat = maxLvChange !== null ? Math.abs(maxLvChange) : 0;
            const clStr = cl !== null ? `${cl.toFixed(1)} pF` : '-';
