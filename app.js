...
// Open Detail Modal and Populate Content
function openDetailModal(serialNumber) {
  const tr = rawData.find(item => item.SERIAL_NUMBER === serialNumber);
  if (!tr) return;
  
  // Set Technical Specs
  document.getElementById('modal-transformer-title').textContent = tr.LOCAL_EQUIPMENT_CODE || 'Transformer Spec';
  document.getElementById('modal-transformer-subtitle').textContent = `Serial: ${tr.SERIAL_NUMBER} | Device Code: ${tr.DEVICE_CODE || '-'}`;
  
  document.getElementById('spec-site').textContent = tr.SITE || '-';
  document.getElementById('spec-company').textContent = tr.COMPANY_CODE || '-';
  document.getElementById('spec-brand').textContent = tr.MANUFACTURER_COMPANY || tr.BRAND || '-';
  document.getElementById('spec-mfg-date').textContent = tr.MANUFACTURING_DATE || '-';
  document.getElementById('spec-power-rating').textContent = tr.POWER_RATING ? `${tr.POWER_RATING} kVA` : '-';
  document.getElementById('spec-cooling').textContent = tr.TYPE_OF_COOLING || '-';
  document.getElementById('spec-tap-changer').textContent = tr.TAP_CHANGER_TYPE || '-';
  document.getElementById('spec-vector-group').textContent = tr.VECTOR_GROUP || '-';
  
  const hv = tr.HV_RATED ? `${tr.HV_RATED} kV` : '-';
  const hva = tr.HV_RATED_A ? ` (${tr.HV_RATED_A} A)` : '';
  document.getElementById('spec-hv-voltage').textContent = hv + hva;
  
  const lv = tr.LV_RATED ? `${tr.LV_RATED} kV` : '-';
  const lva = tr.LV_RATED_A ? ` (${tr.LV_RATED_A} A)` : '';
  document.getElementById('spec-lv-voltage').textContent = lv + lva;
  
  document.getElementById('spec-insulation').textContent = tr.DATA || 'OIL TYPE';
  
  const totalWeight = tr.TOTAL_MASS ? `${tr.TOTAL_MASS} kg` : '-';
  const oilWeight = tr.OIL_MASS ? ` / ${tr.OIL_MASS} kg` : '';
  document.getElementById('spec-mass').textContent = totalWeight + oilWeight;
  
  document.getElementById('spec-gps').textContent = tr.LOCATION_GPS || 'Not configured';
  
  // Health Index Details
  const hi = parseFloat(tr.HI) || 0;
  document.getElementById('modal-health-score').textContent = hi;
  document.getElementById('modal-inspection-date').textContent = tr.Result_Date || '-';
  
  // Set health ring dasharray (circumference is ~100)
  const ring = document.getElementById('modal-health-ring');
  ring.setAttribute('stroke-dasharray', `${hi}, 100`);
  ...
  if (hi === 0 || isNaN(hi)) {
    ...
    recTitle.textContent = 'Normal Operation & Routine Maintenance';
    recText.textContent = 'The transformer health condition is excellent. Perform normal scheduled monitoring and routine visual inspection.';
  } else if (hi >= 70) {
    badge.classList.add('badge-monitoring');
    badge.textContent = 'MONITORING';
    ring.classList.add('monitoring');
    recCard.classList.add('monitoring');
    recTitle.textContent = 'Increased Surveillance & Parameters Monitor';
    recText.textContent = 'Condition is monitoring. Schedule next inspection within 6-12 months. Monitor insulating oil parameters closely and track temperature load trends.';
  } else if (hi >= 50) {
    badge.classList.add('badge-warning');
    badge.textContent = 'WARNING';
    ring.classList.add('warning');
    recCard.classList.add('warning');
    recTitle.textContent = 'Plan Maintenance & Diagnostics';
    recText.textContent = 'Condition is warning. Schedule detailed oil tests (DGA, Furan) and electrical diagnostics (Impedance, insulation resistance). Plan oil filtration or replacement if necessary.';
  } else {
    badge.classList.add('badge-critical');
    badge.textContent = 'CRITICAL';
    ring.classList.add('critical');
    recCard.classList.add('critical');
    recTitle.textContent = 'Immediate Inspection & Offline Investigation';
    recText.textContent = 'Condition is critical. High risk of failure. Immediate offline electrical tests required. Inspect bushings and tap changer. Plan refurbishment or replacement immediately.';
  }
  
  // Sub-scores gauges
  updateSubscoreBar('gi', parseFloat(tr.GI) || 0, 5);
  updateSubscoreBar('api', parseFloat(tr.API) || 0, 20);
  updateSubscoreBar('bi', parseFloat(tr.BI) || 0, 20);
  updateSubscoreBar('ari', parseFloat(tr.ARI) || 0, 20);
  
  // Insulating Oil Inspection (IOI) & Dissolved Gas Analysis (DGA) only apply to Oil Type
  const isOilType = (tr.DATA || '').toUpperCase().includes('OIL');
  const ioiRow = document.getElementById('row-ioi');
  const dgaRow = document.getElementById('row-dga');
  
  if (isOilType) {
    ioiRow.style.display = 'flex';
    dgaRow.style.display = 'flex';
    updateSubscoreBar('ioi', parseFloat(tr.IOI) || 0, 15);
    updateSubscoreBar('dga', parseFloat(tr.DGA) || 0, 20);
  } else {
    ioiRow.style.display = 'none';
    dgaRow.style.display = 'none';
  }
  
  // OLTC Inspection (OLTCI) only applies if Tap Changer is OLTC
  const hasOLTC = (tr.TAP_CHANGER_TYPE || '').toUpperCase().includes('OLTC');
  const oltciRow = document.getElementById('row-oltci');
  if (hasOLTC) {
    oltciRow.style.display = 'flex';
    updateSubscoreBar('oltci', parseFloat(tr.OLTCI) || 0, 20);
  } else {
    oltciRow.style.display = 'none';
  }
